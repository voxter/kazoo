...
-define(APNS, <<"apns">>).
-define(SUPPORTED_PRIVATE_KEY_TYPES, ['RSAPrivateKey'
                                     ,'DSAPrivateKey'
                                     ,'ECPrivateKey'
                                     ,'PrivateKeyInfo'
                                     ]).
...
build_connection(ServerConfig) ->
    ...
    [PemEntry|_] = public_key:pem_decode(KeyPem),
    KeyData = parse_key(PemEntry),
...
-spec parse_key({public_key:pki_asn1_type(), binary(), _}) -> {public_key:pki_asn1_type(), binary()}.
parse_key({KeyType, Key, _}) ->
    case lists:member(KeyType, ?SUPPORTED_PRIVATE_KEY_TYPES) of
        'true' -> {KeyType, Key};
        'false' -> throw('unsupported_private_key_type')
    end.

-spec get_timeout() -> pos_integer().
get_timeout() ->
    kapps_config:get_pos_integer(?CONFIG_CAT, [?APNS, <<"timeout_ms">>], 10000).
...
get_host(<<"dev">>) ->
    ...
    kapps_config:get_string(?CONFIG_CAT, [?APNS, <<"dev_host">>], <<"api.sandbox.push.apple.com">>);
get_host(<<"prod">>) ->
    %% Even though we want a string, the default should still be a binary. It will be converted to a string for us
