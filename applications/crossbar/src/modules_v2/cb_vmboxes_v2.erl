    ...
    case kz_vm_message:message_doc(cb_context:account_id(Context), MediaId) of
        {'ok', JObj} ->
            case kz_datamgr:open_cache_doc(cb_context:account_db(Context), BoxId) of
                {'error', Error} ->
                    crossbar_doc:handle_kz_datamgr_errors(Error, BoxId, Context);
                {'ok', BoxJObj} ->
...
load_attachment_from_message(Doc, BoxId, Context, Timezone) ->
    ...
    of
        'false' -> crossbar_doc:handle_kz_datamgr_errors('not_found', MediaId, Context);
        {'error', Error} ->
            crossbar_doc:handle_kz_datamgr_errors(Error, MediaId, Context);
        {'ok', AttachBin} ->
...
save_attachments_to_file([Id|Ids], BoxId, Context, Timezone, WorkDir) ->
    ...
    try save_attachment_to_file(Id, BoxId, Context, Timezone, WorkDir) of
        ...
        {'error', Error} ->
            _ = del_dir(WorkDir),
            crossbar_doc:handle_kz_datamgr_errors(Error, Id, Context)
    catch
        _:_ ->
            _ = del_dir(WorkDir),
