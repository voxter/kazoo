...
-define(KEY_NOTIFICATION_PREFERENCES, <<"notification_preferences">>).
-define(KEY_NOTIFICATION_TYPE, <<"notification_type">>).
-define(KEY_PUSH, <<"push">>).
-define(KEY_PUSH_NOTIFICATION_SUBSCRIPTIONS, <<"push_notification_subscriptions">>).
...
create(Context, App, DeviceId) ->
    case subscription_does_exist(DeviceId, Context) of
        {'exists', DeviceContext} ->
            lager:error("database already contains push notification subscription for [~p, ~p]", [DeviceId, App]),
            crossbar_util:response_400(<<"subscription already exists for this device">>, <<"bad request">>, DeviceContext);
...
validate_push_notification_subscriptions(Context, App, DeviceContext) ->
    ...
    ReqData = kzd_push_notification_subscriptions:set_app_name(cb_context:req_data(Context), App),
...
on_successful_validation(DeviceContext, Context) ->
    ...
    case cb_context:has_errors(Context1) of
        ...
        'false' ->
            ...
            DeviceDoc = kzd_devices:set_push_notification_subscriptions(cb_context:doc(DeviceContext), cb_context:doc(Context)),
...
validate_notification_preference_uniqueness(Context) ->
    ...
    Subscriptions = kz_json:values(kzd_push_notification_subscriptions:notification_registration_ids(ReqData)),
...
validate_notification_preference_uniqueness_fold(Context, [], _, Dupes) ->
    case sets:size(Dupes) of
        ...
        _ ->
            ...
            Data = kz_json:set_value([?KEY_NOTIFICATION_PREFERENCES, <<"invalid">>], ValidationError, kz_json:new()),
...
validate_notification_preference_uniqueness_fold(Context, [Subscription|Subscriptions], ExistingPrefs, Dupes) ->
    NotifPrefs = kz_json:get_list_value(?KEY_NOTIFICATION_PREFERENCES, Subscription),
...
%% @end
%%------------------------------------------------------------------------------
-spec validate_ios_incoming_call_is_apns(cb_context:context()) -> cb_context:context().
validate_ios_incoming_call_is_apns(Context) ->
    ReqData = cb_context:req_data(Context),
    case kzd_push_notification_subscriptions:platform(ReqData) of
        <<"ios">> ->
            Subscriptions = kz_json:values(kzd_push_notification_subscriptions:notification_registration_ids(ReqData)),
...
-spec validate_ios_incoming_call_is_apns_fold(cb_context:context(), kz_json:objects()) -> cb_context:context().
...
validate_ios_incoming_call_is_apns_fold(Context, [Subscription|Subscriptions]) ->
    NotifPrefs = kz_json:get_list_value(?KEY_NOTIFICATION_PREFERENCES, Subscription),
    NotifType = kz_json:get_ne_binary_value(?KEY_NOTIFICATION_TYPE, Subscription),
    case {lists:member(<<"incoming_call">>, NotifPrefs), NotifType =:= <<"apns">>} of
        {'true', 'false'} ->
            ...
            ValidationError = kz_json:from_list([{<<"message">>, Msg}
                                                ,{?KEY_NOTIFICATION_TYPE, NotifType}
                                                ]),
            Data = kz_json:set_value([?KEY_NOTIFICATION_PREFERENCES, <<"invalid">>], ValidationError, kz_json:new()),
            crossbar_util:response_invalid_data(Data, Context);
...
update_pusher_props(DeviceDoc) ->
    Subscriptions = kzd_devices:push_notification_subscriptions(DeviceDoc),
    case pusher_params_compat(Subscriptions) of
        [] -> kz_json:delete_key(?KEY_PUSH, DeviceDoc);
        Params ->
            Push = kz_json:set_values(Params, kz_json:new()),
            kz_json:set_value(?KEY_PUSH, Push, DeviceDoc)
    end.
...
%% @end
%%------------------------------------------------------------------------------
-spec pusher_params_compat(kzd_push_notification_subscriptions:doc()) -> kz_term:proplist().
pusher_params_compat(Subscriptions) ->
    AppName = kzd_push_notification_subscriptions:app_name(Subscriptions),
    SIPProxyServer = kzd_push_notification_subscriptions:sip_proxy_server(Subscriptions),
    NotifRegs = kzd_push_notification_subscriptions:notification_registration_ids(Subscriptions),
    pusher_params_compat_fold(AppName, SIPProxyServer, kz_json:get_values(NotifRegs)).

-spec pusher_params_compat_fold(kz_term:ne_binary(), kz_term:ne_binary(), {kz_json:objects(), kz_json:keys()}) ->
          kz_term:proplist().
...
-spec purge_existing_subscriptions_for_mobile_device(cb_context:context(), kz_term:ne_binary(), on_purge_success_callback()) ->
          cb_context:context().
purge_existing_subscriptions_for_mobile_device(DeviceContext, DeviceId, OnPurgeSuccess) ->
    Subscriptions = kzd_devices:push_notification_subscriptions(cb_context:doc(DeviceContext)),
    MobileDeviceId = kzd_push_notification_subscriptions:mobile_device_id(Subscriptions),
    ViewContext = crossbar_doc:load_view(?BY_MOBILE_DEVICE, [{'key', MobileDeviceId}], DeviceContext, fun normalize_view_results/2),
    case cb_context:has_errors(ViewContext) of
        'true' -> ViewContext;
        'false' ->
            OtherDeviceDocs = cb_context:doc(ViewContext),
            purge_existing_subscriptions(DeviceContext, DeviceId, OtherDeviceDocs, OnPurgeSuccess)
    end.
...
purge_existing_subscriptions(DeviceContext, DeviceId, [OtherDeviceDoc|OtherDeviceDocs], OnPurgeSuccess) ->
    OtherDeviceId = kz_doc:id(OtherDeviceDoc),
    OnSuccess = fun() -> purge_existing_subscriptions(DeviceContext, DeviceId, OtherDeviceDocs, OnPurgeSuccess) end,
    case OtherDeviceId of
        DeviceId ->
            %% Skipping purging the one we are planning to update, it's a waste and causes conflicts
            OnSuccess();
        _ -> purge_subscriptions(DeviceContext, OtherDeviceId, OtherDeviceDoc, OnSuccess)
    end.

-spec purge_subscriptions(cb_context:context(), kz_term:ne_binary(), kzd_devices:doc(), on_purge_success_callback()) ->
          cb_context:context().
purge_subscriptions(DeviceContext, OtherDeviceId, OtherDeviceDoc, OnSuccess) ->
    ...
    case kz_datamgr:save_doc(cb_context:account_db(DeviceContext), OtherDeviceDoc1) of
        {'ok', _} -> OnSuccess();
        {'error', Error} -> crossbar_doc:handle_datamgr_errors(Error, OtherDeviceId, DeviceContext)
    end.
...
subscription_does_exist(DeviceDoc) ->
    case kzd_devices:push_notification_subscriptions(DeviceDoc) of
...
delete_subscriptions(DeviceDoc) ->
    kz_json:delete_keys([?KEY_PUSH
                        ,?KEY_PUSH_NOTIFICATION_SUBSCRIPTIONS
                        ]
                       ,DeviceDoc
                       ).
