...
publish_event(Props, broken, Socket) ->
    lists:foreach(fun(Part) ->
        gen_tcp:send(Socket, quilt_util:format_prop(Part))
        end, Props),
    gen_tcp:send(Socket, <<"\r\n">>);
publish_event(Props, _, Socket) ->
    %lager:debug("AMI: publish ~p", [Props]),
    gen_tcp:send(Socket, quilt_util:format_binary(Props)).
...
handle_cast({login, AccountId}, State) ->
    {ok, OriginatorPid} = quilt_originator:start_link(),
...
handle_info({tcp, _Socket, Data}, #state{bundle=Bundle,
    ...
    case list_to_binary(Data) of
        <<"\r\n">> ->
            maybe_send_response(quilt_commander:handle(Bundle, AccountId)),
