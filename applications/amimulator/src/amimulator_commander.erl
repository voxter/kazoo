    ...
    {'ok', Results} = couch_mgr:get_results(AccountDb, <<"devices/listing_by_owner">>),
        lists:foldl(fun(Result, Registrations) ->
            ...
            case wh_json:get_value(<<"key">>, Result) of
                'null' ->
                    [reg_entry(AccountId, wh_json:get_value(<<"id">>, Value), wh_json:get_value(<<"name">>, Value), ActionId)] ++ Registrations;
                OwnerId ->
                    case couch_mgr:open_doc(AccountDb, OwnerId) of
                        {'error', 'not_found'} ->
                            ...
                            Registrations;
                        {'ok', Endpoint} ->
                            [reg_entry(AccountId, wh_json:get_value(<<"id">>, Value), wh_json:get_value(<<"username">>, Endpoint), ActionId)] ++ Registrations
                    end
            end
        end, [], Results).
