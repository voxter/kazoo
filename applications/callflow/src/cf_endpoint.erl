...
%% @end
%%--------------------------------------------------------------------
-spec get(kapps_call:call()) ->
                 {'ok', kz_json:object()} |
                 {'error', any()}.
-spec get(api_binary(), ne_binary() | kapps_call:call()) ->
                 {'ok', kz_json:object()} |
                 {'error', any()}.
...
merge_attributes([<<"call_restriction">>|Keys], Account, Endpoint, Owner) ->
    Classifiers = kz_json:get_keys(wnm_util:available_classifiers()),
...
merge_attributes([Key|Keys], Account, Endpoint, Owner) ->
    ...
    OwnerAttr = kz_json:get_ne_value(Key, Owner, kz_json:new()),
    Merged = kz_json:merge_recursive([AccountAttr, EndpointAttr, OwnerAttr]
                                     ,fun(_, V) -> kz_util:is_not_empty(V) end
                                    ),
    merge_attributes(Keys, Account, kz_json:set_value(Key, Merged, Endpoint), Owner).
...
merge_attribute_caller_id(AccountJObj, AccountJAttr, UserJAttr, EndpointJAttr) ->
    ...
    kz_json:merge_recursive(Merging, fun(_, V) -> kz_util:is_not_empty(V) end).
...
fix_user_restrictions(JObj) ->
    lists:foldl(fun(Key, J) ->
                ...
                end, JObj, kz_json:get_keys(wnm_util:available_classifiers())).
...
maybe_create_endpoint(Endpoint, Properties, Call) ->
    case {is_call_forward_enabled(Endpoint, Properties) andalso kz_json:is_true([<<"call_forward">>, <<"substitute">>], Endpoint),
...
-spec is_follow_me_on(kz_json:object()) -> true | false.
is_follow_me_on(Endpoint) ->
...
maybe_record_call(Endpoint, Call) ->
    case is_call_recording(Call) orelse is_sms(Call) of
        'true' -> 'ok';
        'false' -> maybe_start_call_recording(kz_json:get_value(<<"record_call">>, Endpoint, kz_json:new()), Call)
    end.

-spec is_call_recording(kapps_call:call()) -> boolean().
is_call_recording(Call) ->
    case kz_cache:peek_local(?CALLFLOW_CACHE, call_recording_cache_key(Call)) of
        {'ok', _} -> 'true';
        {'error', 'not_found'} -> 'false'
    end.

-spec call_recording_cache_key(kapps_call:call()) ->
                                      {?MODULE, 'recording', ne_binary()}.
call_recording_cache_key(Call) ->
    {?MODULE, 'recording', kapps_call:call_id(Call)}.

-spec maybe_start_call_recording(kz_json:object(), kapps_call:call()) -> 'ok'.
...
-spec start_call_recording(kz_json:object(), kapps_call:call()) -> 'ok'.
start_call_recording(RecordCall, Call) ->
    ...
    Data = kz_json:set_value(<<"spawned">>, 'true', RecordCall),
    _P = kz_util:spawn('cf_record_call', 'handle', [Data, Call]),
...
create_call_fwd_endpoint(Endpoint, Properties, Call) ->
    ...
    Prop = [{<<"Invite-Format">>, <<"route">>}
            ...
            ,{<<"Route">>, <<"loopback/", (kz_json:get_value(<<"number">>, CallForward, <<"unknown">>))/binary, "/context_2">>}
