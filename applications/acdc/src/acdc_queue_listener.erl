...
handle_cast({'timeout_member_call', JObj}, #state{delivery=Delivery
    ...
    publish_queue_member_remove(AccountId, QueueId, whapps_call:call_id(Call)),
...
-spec apply_callback_details(whapps_call:call(), wh_proplist()) ->
                                    whapps_call:call().
apply_callback_details(Call, Props) ->
    case props:get_value(<<"Callback-Details">>, Props) of
        'undefined' -> Call;
        CallbackDetails ->
            CIDPrepend = wh_json:get_value(<<"Prepend-CID">>, CallbackDetails),
            whapps_call:kvs_store('prepend_cid_name', CIDPrepend, Call)
    end.
