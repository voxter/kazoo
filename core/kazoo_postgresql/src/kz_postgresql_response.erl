...
-export([parse_response_to_doc/2
        ,parse_response_to_bulk_response_doc/2
        ,parse_response_to_view_doc/2
        ,format_error/1
        ]).
...
%% @end
%%------------------------------------------------------------------------------
-spec parse_response_to_doc(kz_postgresql:table_name(), epgsql:reply()) -> {'ok', kz_doc:object() | kz_doc:objects()} | kz_data:data_error().
%% Error response
parse_response_to_doc(_TableName, {'error', Cause}=Error) ->
    lager:debug("postgresql query returned error, ~p", [Cause]),
    {'error', format_error(Error)};

%% SELECT with an empty response
parse_response_to_doc(_TableName, {'ok', _Columns, []}) ->
...
parse_response_to_doc(TableName, {'ok', Columns, Rows}) ->
...
parse_response_to_doc(_TableName, {'ok', 0}) ->
...
parse_response_to_doc(_TableName, {'ok', RowCount}) ->
...
parse_response_to_doc(_TableName, {'ok', 0, _, _}) ->
    lager:debug("postgresql query to table '~p' returned an ok result with 0 rows", [_TableName]),
    {'error', 'not_found'};

%% INSERT, UPDATE or DELETE with RETURNING response with 1 or more rows
parse_response_to_doc(TableName, {'ok', _, Columns, Rows}) ->
    lager:debug("postgresql query to table '~p' returned an ok result with ~p rows", [TableName, length(Rows)]),
...
%% @end
%%------------------------------------------------------------------------------
-spec parse_response_to_view_doc(kz_postgresql:table_name() | kz_postgresql:view_name(), epgsql:reply()) ->
          {'ok', kz_doc:objects()} | kz_data:data_error().
parse_response_to_view_doc(_TableOrViewName, {'error', _Cause}=Error) ->
    lager:error("postgresql view query returned an error: ~p", [Error]),
    {'error', format_error(Error)};
parse_response_to_view_doc(_TableOrViewName, {'ok', _Columns, []}) ->
    lager:debug("postgresql view query returned an ok result with 0 rows"),
    {'ok', []};
parse_response_to_view_doc(TableOrViewName, {'ok', Columns, Rows}) ->
    lager:debug("postgresql query returned an ok result with ~p rows", [length(Rows)]),
    Docs = col_and_rows_to_view_jobj(TableOrViewName, Columns, Rows),
...
-spec add_postgresql_value_to_jobj(kz_postgresql:table_name() | kz_postgresql:view_name()
          ...
          kz_json:object().
add_postgresql_value_to_jobj(TableOrViewName, ColumnName, Value, Type, JObj) ->
    add_postgresql_value_to_jobj(TableOrViewName, ColumnName, Value, Type, JObj, []).
...
-spec add_postgresql_value_to_jobj(kz_postgresql:table_name() | kz_postgresql:view_name()
          ...
          kz_json:object().
%% Handle data
add_postgresql_value_to_jobj(_TableOrViewName, <<"data">>, Value, 'jsonb'=Type, JObj, PrePath) ->
