...
%% Drop _rev column as this is handled by the db layer (Query Transaction)
%% Ensure "data" is the last element in the list
%% Any remaining json/jsonb columns are at the end of the list but before "data"
...
sort_and_drop_columns([{<<"data">>, _Type} = DataCol | Columns], OtherCols, JsonCols, _LastCol) ->
    sort_and_drop_columns(Columns, OtherCols, JsonCols, [DataCol]);
...
generate_query_values_and_column_lists([{ColumnName, _Type} | OtherColumns] = ColumnAndTypeList, Values, Columns) ->
    Count = kz_term:to_binary(length(ColumnAndTypeList)),
